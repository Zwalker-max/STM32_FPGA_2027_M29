...
void MainProcess(void)
{
   ...
   */
	...
	while (1)
	{
		if(Test1)
		{
			BSP_UART_Transmit_DMA(&buart1,"Test1\n\n\n");
				...
				Test1 = false;
				BSP_UART_Transmit_DMA(&buart1,"Test1 end\n");
		...
		if(Test2)
		{
			BSP_UART_Transmit_DMA(&buart1,"Test2\n\n\n");
			...
			Test2 = false;
			BSP_UART_Transmit_DMA(&buart1,"Test2 end\n");
		}
		...
		if(Test3)
		{
			BSP_UART_Transmit_DMA(&buart1,"Test3\n\n\n");
			
			
			  /*
			 * FPGA AD9220 dual-ADC capture verification.
			 * Tests:  (1) ADC1 single, (2) ADC2 single, (3) dual synchronous.
			 */
			{
				char       buf[128];
				uint16_t   adc_id;
				...
				uint16_t   act1, act2;
				uint16_t   read_cnt1, read_cnt2;
				const uint16_t demo_pts = 1024;
				int        i;

				HAL_UART_Transmit(&huart1, (uint8_t *)"\r\n=== FPGA AD9220 Dual-ADC Test ===\r\n", 39, 200);

				/* --- ID check (shared for both ADCs) --- */
				...
				if (adc_id != ADC_CAPTURE_ID_VALUE) {
					...
					if (ADC_CaptureStartSingle(1U, demo_pts) != HAL_OK) {
						HAL_UART_Transmit(&huart1, (uint8_t *)"ADC1 start failed.\r\n", 20, 200);
					} else if (ADC_CaptureWaitDone1(200) != HAL_OK) {
						status1 = ADC_CaptureReadStatus1();
						sprintf(buf, "ADC1 wait failed, status=0x%04X\r\n", status1);
						HAL_UART_Transmit(&huart1, (uint8_t *)buf, strlen(buf), 200);
						(void)ADC_CaptureStop();
					} else {
						act1     = ADC_CaptureReadActualCount1();
						read_cnt1 = (act1 > demo_pts) ? demo_pts : act1;

						sprintf(buf, "ADC1 actual = %u, reading %u samples...\r\n", act1, read_cnt1);
						HAL_UART_Transmit(&huart1, (uint8_t *)buf, strlen(buf), 100);

						if (ADC_CaptureReadMDMA1(adc_capture_buffer, read_cnt1, 1000) != HAL_OK) {
							HAL_UART_Transmit(&huart1, (uint8_t *)"ADC1 MDMA read failed.\r\n", 25, 200);
						} else {
							HAL_UART_Transmit(&huart1, (uint8_t *)"ADC1 samples (every 5):\r\n", 25, 200);
							for (i = 0; i < (int)demo_pts; i += 5) {
								sprintf(buf, "  [%04d] : %4u\r\n", i, adc_capture_buffer[i] & 0x0FFFU);
								HAL_UART_Transmit(&huart1, (uint8_t *)buf, strlen(buf), 100);
							}
						}
					}
					...
					if (ADC_CaptureStartSingle(2U, demo_pts) != HAL_OK) {
						HAL_UART_Transmit(&huart1, (uint8_t *)"ADC2 start failed.\r\n", 20, 200);
					} else if (ADC_CaptureWaitDone2(200) != HAL_OK) {
						status2 = ADC_CaptureReadStatus2();
						sprintf(buf, "ADC2 wait failed, status=0x%04X\r\n", status2);
						HAL_UART_Transmit(&huart1, (uint8_t *)buf, strlen(buf), 200);
						(void)ADC_CaptureStop();
					} else {
						act2     = ADC_CaptureReadActualCount2();
						read_cnt2 = (act2 > demo_pts) ? demo_pts : act2;

						sprintf(buf, "ADC2 actual = %u, reading %u samples...\r\n", act2, read_cnt2);
						HAL_UART_Transmit(&huart1, (uint8_t *)buf, strlen(buf), 100);

						if (ADC_CaptureReadMDMA2(adc_capture_buffer2, read_cnt2, 1000) != HAL_OK) {
							HAL_UART_Transmit(&huart1, (uint8_t *)"ADC2 MDMA read failed.\r\n", 25, 200);
						} else {
							HAL_UART_Transmit(&huart1, (uint8_t *)"ADC2 samples (every 5):\r\n", 25, 200);
							for (i = 0; i < (int)read_cnt2; i += 5) {
								sprintf(buf, "  [%04d] : %4u\r\n", i, adc_capture_buffer2[i] & 0x0FFFU);
								HAL_UART_Transmit(&huart1, (uint8_t *)buf, strlen(buf), 100);
							}
						}
					}

					/* ============ Test 3: Dual synchronous ============ */
					HAL_UART_Transmit(&huart1, (uint8_t *)"\r\n--- Test 3: Dual Sync ---\r\n", 30, 200);

					if (ADC_CaptureStartDual(demo_pts) != HAL_OK) {
						HAL_UART_Transmit(&huart1, (uint8_t *)"Dual start failed.\r\n", 20, 200);
					} else if ((ADC_CaptureWaitDone1(200) != HAL_OK) ||
										 (ADC_CaptureWaitDone2(200) != HAL_OK)) {
						status1 = ADC_CaptureReadStatus1();
						status2 = ADC_CaptureReadStatus2();
						sprintf(buf, "Dual wait failed, st1=0x%04X st2=0x%04X\r\n", status1, status2);
						HAL_UART_Transmit(&huart1, (uint8_t *)buf, strlen(buf), 200);
						(void)ADC_CaptureStop();
					} else {
						act1     = ADC_CaptureReadActualCount1();
						...
						sprintf(buf, "Dual: ADC1=%u ADC2=%u\r\n", act1, act2);
						HAL_UART_Transmit(&huart1, (uint8_t *)buf, strlen(buf), 100);

						if ((ADC_CaptureReadMDMA1(adc_capture_buffer,  read_cnt1, 1000) != HAL_OK) ||
								(ADC_CaptureReadMDMA2(adc_capture_buffer2, read_cnt2, 1000) != HAL_OK)) {
							HAL_UART_Transmit(&huart1, (uint8_t *)"Dual MDMA read failed.\r\n", 24, 200);
						} else {
							HAL_UART_Transmit(&huart1, (uint8_t *)"Dual samples (every 5):\r\n", 25, 200);
							for (i = 0; i < (int)demo_pts; i += 5) {
								BSP_UART_Transmit_DMA(&buart1, "  [%04d] ADC1 ADC2:%4u , %4u\r\n",
												i,
												adc_capture_buffer[i]  & 0x0FFFU,
												adc_capture_buffer2[i] & 0x0FFFU);
							}
						}
					}

					HAL_UART_Transmit(&huart1, (uint8_t *)"\r\n=== Dual-ADC Test Complete ===\r\n", 35, 200);
				}
			}
			...
			Test3 = false;
			BSP_UART_Transmit_DMA(&buart1,"Test3 end\n");
		}
		...
		if(Test4)
		{
			BSP_UART_Transmit_DMA(&buart1,"Test4\n\n\n");
			Test4 = false;
			BSP_UART_Transmit_DMA(&buart1,"Test4 end\n");
		}
